...
import {
	...
	listFiles,
	normalisePhotos,
} from "../../src/_lib/google-drive.js";

// The Google half of the gallery: transport and normaliser, nothing about how
// annie-elliot displays a photo. `listFiles` takes its `fetch` as an option, so
// nothing here touches `globalThis.fetch` or the network — and `normalisePhotos`
// is pure, so the interesting half needs no seam at all.
...
test("caption is never empty: the description, else the filename", () => {
	...
	// The package falls back to the RAW name; prettifying it is a site
	// convention and lives in src/_data/gallery.js.
...
test("mimeType is returned and never filtered on", () => {
	// SKIP_MIME is annie-elliot's policy, not Google's — the transport hands
	// everything over and the site decides.
	const photos = normalisePhotos({
		files: [
...
test("an empty listing returns [] rather than throwing", async () => {
	// Deliberately unlike src/_data/gallery.js: `200 {"files": []}` is what a
	// sharing change looks like *for this folder*, and only the site knows that.
	const { fetch } = serve({ files: [] });
